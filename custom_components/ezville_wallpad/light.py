...
_LOGGER = logging.getLogger("custom_components.ezville_wallpad.light")
...
class EzvilleLight(CoordinatorEntity, LightEntity):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        _LOGGER.debug("===> Light entity %s received update callback, current_state: %s", 
                     self._attr_name, current_state)
        
        # Schedule update safely from any thread
        if hasattr(self, 'hass') and self.hass:
            _LOGGER.debug("===> Scheduling state write for light entity %s", self._attr_name)
